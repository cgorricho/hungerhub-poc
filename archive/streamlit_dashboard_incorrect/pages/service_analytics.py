...
def render_service_analytics():
    ...
    # Date range filter
    min_date = services_df['service_date'].min().date()
    max_date = services_df['service_date'].max().date()
    ...
    # Service type filter
    service_types = ['All'] + list(services_df['service_type'].unique())
    ...
    if selected_service != 'All':
        filtered_services = filtered_services[filtered_services['service_type'] == selected_service]
    ...
    with col1:
        ...
        daily_services = filtered_services.groupby(
            filtered_services['service_date'].dt.date
        ).size().reset_index()
    ...
    with col2:
        ...
        service_counts = filtered_services['service_type'].value_counts()
        fig = px.bar(
    ...
    with col1:
        ...
        if 'day_of_week' in filtered_services.columns:
            ...
            filtered_services['day_of_week'] = filtered_services['service_date'].dt.day_name()
            dow_counts = filtered_services['day_of_week'].value_counts()
    ...
    with col2:
        ...
        monthly_counts = filtered_services.groupby(
            filtered_services['service_date'].dt.month
        ).size()
    ...
    if 'foodpounds' in filtered_services.columns:
        ...
        with col1:
            food_by_service = filtered_services.groupby('service_type')['foodpounds'].agg(['sum', 'mean']).round(1)
    ...
    for service_type in filtered_services['service_type'].unique():
        type_services = filtered_services[filtered_services['service_type'] == service_type]
    ...
    if st.button("📊 Export Analysis Report"):
        ...
        report_data = {
            ...
            'service_type_filter': selected_service,

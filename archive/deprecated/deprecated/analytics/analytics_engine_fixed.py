#!/usr/bin/env python3
"""
HungerHub POC - Analytics Engine (Fixed Version)
...
class HungerHubAnalytics:
    ...
    def load_data(self):
        ...
        try:
            ...
            self.services_df = pd.read_csv(f'{self.data_path}/services.csv')
    ...
    def generate_recommendations(self) -> dict:
        ...
        recommendations['service_optimization'] = [
            ...
            f"Most common service type represents the highest volume. Consider expanding capacity for popular services.",
    ...
    def create_visualizations(self):
        ...
        bars = plt.bar(range(len(service_counts)), service_counts.values, color='skyblue', edgecolor='navy', linewidth=1)
        ...
        bars = plt.bar(household_dist.index, household_dist.values, color='lightcoral', edgecolor='darkred', linewidth=1)
        ...
        # Add value labels
        for bar, value in zip(bars, household_dist.values):
            plt.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.05, str(value), 
                    ha='center', va='bottom', fontweight='bold')
        ...
        bars = plt.barh(range(len(food_by_service)), food_by_service.values, color='orange', edgecolor='darkorange', linewidth=1)
        plt.title('Total Food Distribution by Service Type (lbs)', fontsize=16, fontweight='bold')
        plt.xlabel('Total Food Distributed (lbs)', fontsize=12)

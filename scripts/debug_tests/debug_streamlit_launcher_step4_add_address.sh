...
# STEP 4 DEBUG: Adding --server.address parameter using space format
echo "🔍 STEP 4: Adding --server.address parameter (space format)"
...
if command -v lsof >/dev/null 2>&1; then
  if lsof -i TCP:"$PORT" -sTCP:LISTEN >/dev/null 2>&1; then
    ...
    PORT=8599
    PORT_INT=$((PORT))  # Update PORT_INT when PORT changes
  fi
fi

echo "🚪 PORT='$PORT' (from env var with default)"
echo "🚪 PORT_INT='$PORT_INT' (integer conversion)"
echo "🏠 ADDRESS='$ADDRESS' (from env var with default)"

# STEP 4 CLI_ARGS - Adding --server.address using SPACE format
CLI_ARGS=(
  run
...
# Launch Streamlit with port + address
